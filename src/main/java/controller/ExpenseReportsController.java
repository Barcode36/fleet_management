...
import main.java.model.ExpenseCategory;
...
public class ExpenseReportsController {
    ...
    private void initialize() {
        ...
        expenseCategoryChoiceBox.getItems().add(0, null);
    }
    ...
    private void onGenerateGraph() {
        ...
        if (dates == null) {
            AlertUtil.showAlert("", "Choose a range of dates or select a month and year!", Alert.AlertType.ERROR);
        } else {
    ...
    private void getGraphData() {
        Task<XYChart.Series<String, Double>> task = new Task<XYChart.Series<String, Double>>() {
            ...
            protected XYChart.Series<String, Double> call() throws Exception {
                ...
                if (fleetOwnerChoiceBox.getSelectionModel().getSelectedIndex() > 0 && regNumberChoiceBox.getValue() == null) {
                    series.setName(fleetOwnerChoiceBox.getValue().toString() + " Expenses");
                }
        ...
        task.setOnSucceeded(event -> {
            displayGraph(task.getValue());
        });
        new Thread(task).start();
    }

    private void displayGraph(XYChart.Series<String, Double> series) {
        try {
            ...
            controller.setParameters("Date", "Expense");
    ...
    private ObservableList<Map<String, String>> getExpenses(List<LocalDate> dates) throws SQLException {
        String sql = "select ledger_entry.*, drivers.first_name, drivers.national_id, conductors.national_id,  " +
                ...
                "and income = 0 and expense != 0";
        if (expenseCategoryChoiceBox.getValue() != null) {
            sql += " and ledger_entry.category = '" + expenseCategoryChoiceBox.getValue().name() + "'";
        }
        ...
        sql += " order by date";
        return Dao.getCashEntries(sql, LedgerEntryType.EXPENSE);
    }
    ...
    private void onExportReport() {
        ...
        if (dates == null) {
            ...
            task.setOnSucceeded(event -> {
                ...
                params.put("total", CurrencyUtil.formatCurrency(getTotalExpense()));
                params.put("data", getExpenseValues());
                new PrintReport().showReport(params, "expense_report");
            });
            new Thread(task).start();
        }
    }

    private List<Map<String, String>> getExpenseValues() {
        ...
        for (Map<String, String> item : searchResults) {
            ...
            paramMap.put("details", item.get(EXPENSE_DETAIL));
    ...
    private void showSearchResults() {
        ...
        label.getStyleClass().addAll("fw-500", "color-danger");
        ...
        try {
            ...
            controller.setParameters(searchResults, LedgerEntryType.EXPENSE, false);
        ...
        label = new Label("Ksh. " + CurrencyUtil.formatCurrency(getTotalExpense()));
        label.getStyleClass().addAll("h6", "fw-500", "color-danger");
        hBox.getChildren().add(label);
